...
sub ExecutionEnvironment(%)                                                     # Execution environment for a block of code.
 {my (%options) = @_;                                                           # Execution options
  ...
  my $exec=                 genHash(q(Zero::Emulator),                          # Emulator execution environment
    ...
    ResizeRequired=>        0,                                                  # For a given memory access system indicates whether an array's length must be explicitly maintained by the programmer(1) or whether it can be left to Perl(0)
    ResizeUp=>              undef,                                              # Increment the length of an array  by one.
    ResizeDown=>            undef,                                              # Decrement the length of an array  by one.
...
sub popMemoryArea($$)                                                           #P Pop a value from the specified memory area if possible else confess.
 {my ($exec, $area) = @_;                                                       # Execution environment, array,
  @_ == 2 or confess "Two parameters";
  my $a = $exec->memory->[arenaHeap][$area];
  if (!defined($a) or !$a->@*)                                                  # Area does not exists or has zero elements
   {stackTraceAndExit($exec, "Cannot pop area: $area");
   }
  pop @$a;                                                                      # Pop
 }
...
sub setOriginalMemoryTechnique($)                                               #P Set the handlers for the original memory allocation technique.
 {my ($exec) = @_;                                                              # Execution environment
  ...
  $exec->ResizeRequired    = 0;                                                 # Allow Perl to maintain arrays sizes
...
sub stringResizeUp($$)                                                          #P Increment the length of an array
 {my ($exec, $area) = @_;                                                       # Execution environment, arena, array, value to assign
  @_ == 2 or confess "Two parameters";
  ...
  $l < $u-1 or confess "Area overflow, area: $area, position: $l";              # Check we are within the area
...
sub stringPushMemoryArea($$$)                                                   #P Push a value onto the specified array.
 {my ($exec, $area, $value) = @_;                                               # Execution environment, arena, array, value to assign
  ...
  stringResizeUp $exec, $area;                                                  # Increase size of area
 }

sub stringResizeDown($$)                                                        #P Decrement the length of an array
 {my ($exec, $area) = @_;                                                       # Execution environment, array,
  @_ == 2 or confess "Two parameters";
  my $w = $exec->memoryStringElementWidth;                                      # Width of each element in an an area
  my $t = $exec->memoryStringTotalElements;                                     # Total width of a heap area
  my $o = $area * $t;                                                           # Offset of heap area in arena
  my $l = vec($exec->memoryString, $o, $w * 8);                                 # Zero current length of area so we can push and pop
    $l > 0 or confess "Area underflow on area: $area";                          # Check we are within the area
  --$l;                                                                         # Pop
...
sub stringPopMemoryArea($$)                                                     #P Pop a value from the specified memory area if possible else confess.
 {my ($exec, $area) = @_;                                                       # Execution environment, array,
  ...
  stringResizeDown($exec, $area);                                               # Decrease size of area
  $v
 }
...
sub setStringMemoryTechnique($)                                                 #P Set the handlers for the string memory allocation technique.
 {my ($exec) = @_;                                                              # Execution environment
  ...
  $exec->ResizeRequired    = 1;                                                 # We will have to maintain array sizes, either explicitly with extra instructions where it is not obvious what the new length should be or by extending existing instructions where it is obvious what the new length shpuld be
  $exec->ResizeUp          = \&stringResizeUp;                                  # Increase the length of an array
  $exec->ResizeDown        = \&stringResizeDown;                                # Decrease the length of an array
...
$t->dump;   ## arena:
      assign($exec, $t, $s);
      $exec->ResizeUp->($exec, $t->area) if defined $exec->ResizeUp;
     },
    ...
    shiftDown=> sub                                                             # Shift an element down in a memory area
     {my $i = currentInstruction $exec;
      ...
      my $T = $exec->latestLeftTarget;
      assign($exec, $T, $v);
      $exec->ResizeDown->($exec, $T->area) if defined $exec->ResizeDown;
     },
...
confess "Assign of -1".join "\n", $instruction->contextString if (($exec->lastAssignValue//0) == -1);
say STDERR dump($exec->lastAssignValue);
...
sub formatTrace($)                                                              #P Describe last memory assignment.
 {my ($exec) = @_;                                                              # Execution
  ...
  return "" unless defined(my $Type  = $exec->lastAssignType);
  return "" unless defined(my $type  = $exec->block->ArrayNumberToName($Type));
...
if (1)                                                                          ##ShiftUp ##Parallel
 {Start 1;
  ...
  is_deeply [$e->timeParallel, $e->timeSequential], [5,7];
 }
...
if (1)                                                                          ##Alloc ##Mov ##Jeq ##Jne ##Jle ##Jlt ##Jge ##Jgt
 {Start 1;
  ...
  is_deeply $e->analyzeExecutionResults(doubleWrite=>3), "#       26 instructions executed";
...
if (1)                                                                          ##Alloc ##Mov
 {Start 1;
  ...
  is_deeply $e->analyzeExecutionResults(doubleWrite=>3), "#       20 instructions executed";
...
0004    resize [undef, 0, 3, 0]  [undef, 3, 3, 0]  [undef, 3, 3, 0]
0005  arrayDump [undef, 0, 3, 0]  [undef, 0, 3, 0]  [undef, 0, 3, 0]
END
 }
